...
describe("MetaBlox batchMintMetaBlox tests", () => {
  /**
   * =====================================================================
  ...
  it("Should not allow address without correct role to call function", async () => {
    ...
    await expect(
      MetaBloxContract.connect(Alice).batchMintMetaBlox(Bob.address, [1], [1])
    ).to.be.reverted;
  });
  ...
  it("Should not allow minting to the zero address", async () => {
    ...
    await expect(
      MetaBloxContract.batchMintMetaBlox(ZERO_ADDRESS, [1], [1])
  ...
  it("Should recognise a token ID that doesn't exist", async () => {
    ...
    await expect(
      MetaBloxContract.batchMintMetaBlox(Alice.address, [1, 2, 10], [1, 1, 1])
  ...
  it("Should not allow minting an amount less than one", async () => {
    ...
    await expect(
      MetaBloxContract.batchMintMetaBlox(Alice.address, [1, 2, 3], [1, 0, 1])
    ).to.be.revertedWithCustomError(MetaBloxContract, "NotPositiveValue");
  });

  /**
   * =====================================================================
   *   SHOULD BATCH MINT TOKENS
   * =====================================================================
   */
