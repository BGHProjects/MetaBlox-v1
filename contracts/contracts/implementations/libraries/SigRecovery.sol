...
library SigRecovery {
    ...
    function prefixMessageHash(bytes32 _message) internal pure returns (bytes32 prefixedMessageHash){
    ...
    function splitSignature(bytes memory _sig) internal pure returns (bytes32 r, bytes32 s, uint8 v) {
        ...
        assembly {
            // First 32 bytes hold the signature length, skips first 32 bytes as that is the prefix
            r := mload(add(_sig, 32))
    ...
    /// @dev This method prefixes the provided message hash, splits the signature and uses ecrecover ro return the signing address
    /// @param _message The message that was signed
    /// @param _signature The signature
    function recoverAddressFromMessage(bytes memory _message, bytes memory _signature) internal pure returns (address recoveredAddress){
